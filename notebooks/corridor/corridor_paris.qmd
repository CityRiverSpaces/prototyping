...
In this notebooks we explore how to delineate river corridor edges using Paris as the study area. We focus on the Seine and use a specific projected CRS for the analysis.

```{r}
city_name <- "Paris"
river_name <- "La Seine"
epsg_code <- 32631  # UTM zone 31N
# bbox_buffer <- 0  # drop expanded bbox - already "too large" area
```
...
```
```{r}
# get city boundary - tag not available for Paris
...
The river within the bbox defined using the administrative bbox of Paris has too many turns - let's use a reduced bbox.

```{r}
bbox <- c(2.26, 48.84, 2.40, 48.885)
names(bbox) <- c("xmin", "ymin", "xmax", "ymax")
bbox <- st_bbox(bbox, crs = st_crs(4326)) |>
    st_as_sfc() |>
    st_transform(epsg_code)
...
```{r}
area <- areas[4] # determined  visually
