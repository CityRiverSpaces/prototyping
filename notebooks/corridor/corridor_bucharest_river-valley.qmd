...
add_weights <- function(net, geom = NULL, buffer = NULL) {
    edges <- net |> st_as_sf("edges")

    lengths <- st_length(edges)
    net <- activate(net, "edges") |> mutate(length = lengths)

    if (!is.null(geom)){
        distances <- st_distance(edges, geom, which = "Euclidean")
        weight_name <- sprintf("weight_%s", names(geom))
    } else {
        distances <- 0.
        weight_name <- "weight"
    }
    ...
    net |>
        activate("edges") |>
        mutate("{weight_name}" := lengths + distances + repellance)
}
...
```{r}
get_shortest_path <- function(net, end_points, weight_name = "weight"){
    ...
    corridor_edge <- edges[edge_path]
    if (length(corridor_edge) > 1) {
        return(st_union(corridor_edge) |> st_line_merge())
    } else {
        return(corridor_edge)
    }

}

corridor_edge_1 <- get_shortest_path(network_split, end_points_side_1,  weight_name = "weight_side_1")
names(corridor_edge_1) <- "side_1"
corridor_edge_2 <- get_shortest_path(network_split, end_points_side_2,  weight_name = "weight_side_2")
...
corridor_edge_1_nodev <- get_shortest_path(network_split_2, end_points_side_1,  weight_name = "weight_side_1")
names(corridor_edge_1_nodev) <- "side_1"
corridor_edge_2_nodev <- get_shortest_path(network_split_2, end_points_side_2,  weight_name = "weight_side_2")
names(corridor_edge_2_nodev) <- "side_2"
```

We run the procedure again to verify the procedure is actually "converged":
...
corridor_edge_1_nodev_2 <- get_shortest_path(network_split_3, end_points_side_1,  weight_name = "weight_side_1")
corridor_edge_2_nodev_2 <- get_shortest_path(network_split_3, end_points_side_2,  weight_name = "weight_side_2")
```
...
end_points_start <- c(end_points_side_1[1], end_points_side_2[1])
end_points_end <- c(end_points_side_1[2], end_points_side_2[2])
```

Option 1: we close the corridor's polygon by directly connecting the end points of the corridor edges:

```{r}
to_linestring <- function(pts) st_union(pts) |> st_cast("LINESTRING")
to_polygon <- function(...) {
    c(...) |>
        st_union() |>
        st_line_merge() |>
...
edge_3 <- to_linestring(end_points_start)
edge_4 <- to_linestring(end_points_end)
corridor <- to_polygon(corridor_edge_1_nodev, corridor_edge_2_nodev, edge_3, edge_4)
```

Option 2: we close the corridor using the shortest-path method on both ends of the corridor:

```{r}
edge_3 <- get_shortest_path(network_weights, end_points_start, "length")
edge_4 <- get_shortest_path(network_weights, end_points_end, "length")
corridor_network_capped <- to_polygon(corridor_edge_1_nodev, corridor_edge_2_nodev, edge_3, edge_4)
```
...
leaflet() |>
    ...
    addPolygons(data = corridor |> get_geom_latlon(), color = "black", weight = 10) |>
    addPolygons(data = corridor_network_capped |> get_geom_latlon(), color = "red", weight = 10)
```
...
st_write(
    c(st_geometry(corridor_network_capped), corridor_edge_1_nodev, corridor_edge_2_nodev),
    dsn = file_name,
    append=FALSE,
    quiet = TRUE,
)
