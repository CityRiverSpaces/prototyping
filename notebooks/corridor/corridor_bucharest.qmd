...
bbox_expanded_latlon <- bbox_expanded |> st_transform(4326) |> st_bbox()
```
...
get_corridor_edge <- function(network, area, vertices, weight_name = "weight"){
...
```{r}
add_edge_dist <- function(net, geometry, weight_name, weight_distance = 0.6){
    edges <- getEdges(net)
...
corridor_edge_1_weights <- get_corridor_edge(network, area, vertices,  weight_name = "weight_edge_1")

# second edge
edges_initial_2 <- edges_initial[2]
area <- areas[st_intersects(edges_initial_2, areas) |> unlist() ]
corridor_edge_2_weights <- get_corridor_edge(network, area, vertices, weight_name = "weight_edge_2")
```

Cap and compare to the corridor computed without accounting for the distance from the initial guess:

```{r}
corridor_edges_weights <- st_union(corridor_edge_1_weights, corridor_edge_2_weights)

corridor_weights <- city_boundary |>
    st_split(corridor_edges_weights) |>
    st_collection_extract("POLYGON") |>
    st_as_sf() |>
    st_filter(waterway, .predicate = st_intersects)
```
...
leaflet() |>
    addTiles() |>
    addPolylines(data = corridor_edges_weights |> getGeomLatLon())
    # addPolygons(data = corridor |> getGeomLatLon(), color = "red") |>
    # addPolygons(data = corridor_weights |> getGeomLatLon(), color = "red")
```
